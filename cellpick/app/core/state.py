...
This module provides the AppStateManager class that manages the entire
application state, including shapes, landmarks, active regions, and UI state.
"""
...
class AppStateManager:
    ...
    def __init__(self) -> None:
        """Initialize the AppStateManager with default values."""
    ...
    def select_shapes(self, k: int) -> None:
        ...
        if len(active_ids_for_selection) <= k:
            ...
            QMessageBox.warning(
                self.main_window,
                "Warning",
                f"Could not select {k} shapes, selected {len(self.active_shape_ids)}",
            )
            ...
            # Select randomly
            selected_ids = sorted(
                random.sample(range(len(active_ids_for_selection)), k)
            )
            self.selected_shape_ids = [
                active_ids_for_selection[i] for i in selected_ids
            ]
            ...
            if any([len(ids) < k for ids in selected_idss]):
                QMessageBox.warning(
                    self.main_window,
                    "Warning",
                    f"Could not select {k} shapes per label",
                )
            self.selected_shape_ids = []
            for i, selected_ids in enumerate(selected_idss):
                self.selected_shape_ids += [point_ids[i][idx] for idx in selected_ids]
    ...
    def load_cell_labels(self, labels_dict: dict[int, Any]) -> None:
        ...
        labels_dict : dict[int, Any]
            Dictionary mapping cell original IDs (from segmentation mask or table
            instance column) to their labels.
        """
        # If shapes have original_id attributes, build a reverse mapping
        ...
        if has_original_ids:
            ...
            # No original_ids, use labels_dict as-is
